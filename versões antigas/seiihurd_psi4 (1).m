...
%% ============ Load Data ============
load 'input_data/yourmatfile.mat'
...
Cases_dado      = coviddata(1:193,1)';
Cases_dado      = cell2mat(Cases_dado);
R_dado          = coviddata(1:193,7)';
R_dado          = cell2mat(R_dado);
D_dado          = coviddata(1:193,5)';
D_dado          = cell2mat(D_dado);
H_dado          = coviddata(1:193,2)';
H_dado          = cell2mat(H_dado);
U_dado          = coviddata(1:193,3)';
U_dado          = cell2mat(U_dado);
V_dado          = coviddata(1:193,8)';
V_dado          = cell2mat(V_dado);
data_in = datetime(2021,01,01);
Nw_cases        = diff(Cases_dado);
Nw_deaths       = diff(D_dado);
Nw_vac          = diff(V_dado);
Nw_vac          = [Nw_vac,Nw_vac(192)];

t_vac           = 19; %% day started the vaccination
%% ============ Initial Condition ============
...
U0          = U_dado(1);
UV0         = 0;
V0          = V_dado(1);  %% new compartment
Rv0         = 0; %% new compartment
Is0         = (Cases_dado(1)-451240);
Ia0         = Is0*0.9;
Iav0        = 0; %% new compartment
Isv0        = 0; %% new compartment
E0          = (518955-Cases_dado(1));
Ev0         = 0; %% new compartment
S0          = (N0-Is0-Ia0-E0);
Nw0         = 0;
NwV0        = 0;
Model_0     = [S0,E0,V0,Ev0,Ia0,Is0,Iav0,Isv0,H0,HV0,U0,UV0,R0,Rv0,D0,Nw0,NwV0]/N0;

%% ============ Model ============
    
tsim = length(U_dado)-1; 
    
[t,y] = ode45(@(t,y)covid_odes(t,y,Psif,Nw_vac),0:1:tsim,Model_0');
...
%% ============ Model simulation ============
function dydt = covid_odes(t,y,Psif,Nw_vac)
N=14930634;
psi=Psif(floor(t+1));
tau= Nw_vac(floor(t+1));
